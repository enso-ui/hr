<template>
  <div class="wrapper">
    <div v-if="ready" class="columns is-centered is-multiline">
    ...
    >
      <template v-slot:global-actions>
        <v-popover trigger="click" placement="top" ref="popover">
          <a
            v-if="hasSelected"
            class="button has-margin-left-small has-background-info has-text-white"
          >
            <span class="is-hidden-mobile">{{ i18n('Splits') }}</span>
      ...
      <template v-slot:row-actions="{row}">
        <v-popover trigger="click" placement="top" :ref="`popover-${row.id}`">
            ...
            <div style="min-height: 35px; min-width: 350px;">
              ...
              >
                <template v-slot:actions-left="{processing}">
                  <a
                    class="button is-info has-margin-left-medium"
